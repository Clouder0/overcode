...
Hide a subagent from the `@` autocomplete menu with `hidden: true`. Useful for internal subagents that should only be invoked programmatically by other agents via `subagent_spawn`.
...
This only affects user visibility in the autocomplete menu. Hidden agents can still be invoked by the model via `subagent_spawn` if permissions allow.
...
Control which subagents an agent can invoke via `subagent_spawn` with `permission.subagent_spawn_agent`. Uses glob patterns for flexible matching.
...
When set to `deny`, the subagent is removed from the `subagent_spawn` tool description entirely, so the model won't attempt to invoke it.
...
:::tip
Users can always invoke any subagent directly via the `@` autocomplete menu, even if the agent's subagent permissions would deny it.
:::
