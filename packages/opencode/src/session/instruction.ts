...
export namespace InstructionPrompt {
  ...
  export async function system() {
    ...
    const fetches = urls.map(async (url) => {
      ...
      const now = Date.now()
      if (cached && now - cached.at < urlTtlMs) return cached.value

      const value = await fetch(url, { signal: AbortSignal.timeout(5000) })
        .then((res) => (res.ok ? res.text() : ""))
        .catch(() => "")
        .then((x) => (x ? "Instructions from: " + url + "\n" + x : ""))

      urlCache.set(url, { at: now, value })
      return value
    })
