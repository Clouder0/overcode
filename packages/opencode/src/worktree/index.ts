...
export namespace Worktree {
  ...
  export const RemoveInput = z
    .object({
      directory: z.string(),
      confirm: z.literal(true).describe("Set to true to confirm this destructive worktree removal."),
    })
    .meta({
  ...
  export const ResetInput = z
    .object({
      directory: z.string(),
      confirm: z.literal(true).describe("Set to true to confirm this destructive worktree reset."),
    })
    .meta({
  ...
  export const remove = fn(RemoveInput, async (input) => {
    ...
    if (input.confirm !== true) {
      throw new RemoveFailedError({ message: "Confirmation required (set confirm=true)" })
    }
  ...
  export const reset = fn(ResetInput, async (input) => {
    ...
    if (input.confirm !== true) {
      throw new ResetFailedError({ message: "Confirmation required (set confirm=true)" })
    }
    ...
    const devCheck = await $`git show-ref --verify --quiet refs/heads/dev`.quiet().nothrow().cwd(Instance.worktree)
    const masterCheck = await $`git show-ref --verify --quiet refs/heads/master`
      .quiet()
      .nothrow()
      .cwd(Instance.worktree)
    const localBranch =
      mainCheck.exitCode === 0 ? "main" : devCheck.exitCode === 0 ? "dev" : masterCheck.exitCode === 0 ? "master" : ""
