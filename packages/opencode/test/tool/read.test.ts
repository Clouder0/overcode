...
describe("tool.read truncation", () => {
  test("truncates large file by bytes and sets truncated metadata", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(
          result.output.includes("Output truncated at") ||
            result.output.includes("The tool call succeeded but the output was truncated"),
        ).toBe(true)
      },
    })
  })
  ...
  test("applies transport truncation when formatted read output exceeds tool cap", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const line = "x".repeat(20)
        const content = Array.from({ length: 2000 }, () => line).join("\n")
        await Bun.write(path.join(dir, "wide.txt"), content)
      },
    })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const read = await ReadTool.init()
        const result = await read.execute({ filePath: path.join(tmp.path, "wide.txt") }, ctx)
