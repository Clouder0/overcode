...
`,
  ...
  test("hidden same-parent assistant attempts outside transformed prompt scope do not seed dedupe", async () => {
    await using tmp = await tmpdir({
      git: true,
      init: async (dir) => {
        await Bun.write(
        ...
        const processorSpy = spyOn(SessionProcessor, "create").mockImplementation((args: any) => {
          return {
            ...
            async process(input: any) {
              ...
              const meta = out.metadata as { applied?: unknown; reason?: unknown; status?: unknown }
              seen.applied = meta.applied
              seen.reason = meta.reason
              seen.status = meta.status
