...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const isSubagentSession = createMemo(() => {
    const session = info()
    return session?.title?.startsWith("Subagent - ") === true
  })

  const effectiveAgent = createMemo(() => {
    const session = info()
    const name = session?.title?.startsWith("Subagent - ") ? session.title.slice(11) : undefined
    if (!name) return local.agent.current()
    return sync.data.agent.find((agent) => agent.name === name) ?? local.agent.current()
  })
  ...
  const effectiveModel = createMemo(() => {
    ...
    if (isSubagentSession() && agentModel) {
      const provider = providers.all().find((p) => p.id === agentModel.providerID)
      if (provider) {
  ...
  const handleSubmit = async (event: Event) => {
    ...
    const currentAgent = effectiveAgent()
          ...
          <div class="flex items-center justify-start gap-0.5">
            <Switch>
              ...
              <Match when={store.mode === "normal"}>
                ...
                <Show when={!isSubagentSession() && local.model.variant.list().length > 0}>
                  <TooltipKeybind
                    placement="top"
